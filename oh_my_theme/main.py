    ...
    try:
        with request.urlopen(API_URL) as response:
            if response.status == 200:
                ...
                theme_files = [
                    item["name"] for item in data if item["name"].endswith(".omp.json")
                ]
                official_themes = [
                    theme.replace(".omp.json", "") for theme in theme_files
                ]
    ...
    for repo_url in custom_repos:
        try:
            ...
            if not error_msg and theme_files:
                custom_themes = [
                    theme_file["name"].replace(".omp.json", "")
...
def get_active_theme(config_file):
    ...
    try:
        ...
        init_pattern = re.compile(
            r"^\s*(eval.*oh-my-posh init.*|oh-my-posh init.*fish.*)",
        )
...
def _generate_description_from_segments(segment_types) -> str:
    ...
    if key_segments:
        return f"Features: {', '.join(key_segments[:3])}" + (
            "..." if len(key_segments) > 3 else ""
        )
    ...
    for i in range(menu_height):
        ...
        if list_idx < len(items):
            ...
            if selected_items is not None:  # Remote themes
                ...
                active_theme and item_name == active_theme
            ):  # Local themes - mark active
                prefix = "★ "
            ...
            if current_line:
                ...
                if remaining_parts:
                    ...
                    if available_space > 6:  # Minimum space for "... | x"
                        truncated_remaining = (
                            remaining_text[: available_space - 3] + "..."
                        )
...
def main_ui(stdscr) -> None:
    ...
    while True:
        ...
        current_local = (
            search_state["filtered_local"] if search_state["active"] else local_themes
        )
        current_remote = (
            search_state["filtered_remote"] if search_state["active"] else remote_themes
        )
        ...
        if active_list:  # Only handle scrolling if list is not empty
            ...
            if selections[active_panel_idx] < scroll_tops[active_panel_idx]:
                ...
                selections[active_panel_idx]
                >= scroll_tops[active_panel_idx] + menu_height
            ):
                scroll_tops[active_panel_idx] = (
                    selections[active_panel_idx] - menu_height + 1
                )

        # Draw panels with search highlighting
        search_query = search_state["query"] if search_state["active"] else None
        draw_panel_with_search(
            local_panel,
            "Local Themes",
            current_local,
            selections[0],
                ...
                if is_local_theme:
                    ...
                    # Ask permission before downloading
                        ...
                        if keep_theme:
                            ...
                            show_status_message(
                                stdscr,
                                f"Theme '{selected_theme}' downloaded and kept!",
                            )
                        # User wants to remove the theme
                        elif remove_theme(selected_theme):
                            show_status_message(
                                stdscr,
                                f"Theme '{selected_theme}' downloaded but removed!",
                            )
                        else:
                            show_status_message(
                                stdscr,
                                f"Failed to remove downloaded theme '{selected_theme}'",
                            )
                    else:
                        show_status_message(
                            stdscr,
